...
def get_doctors(
    ...
    doctors = crud.get_doctors(db=db, skip=skip, limit=limit, search=search, clinic_id=clinic_id)
    return doctors
...
def add_doctor_to_favorites(
    ...
    crud.add_to_favorites(db=db, user_id=current_user.id, doctor_id=doctor_id)
...
def remove_doctor_from_favorites(
    ...
    crud.remove_from_favorites(db=db, user_id=current_user.id, doctor_id=doctor_id)
